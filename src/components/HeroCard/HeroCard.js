...
// ======== Integração da Avaliação de Heróis no Componente HeroCard
...
import { HeadingTwo } from '../../common-components/HeadingTwo/HeadingTwo';
import { ButtonLink } from '../../common-components/ButtonLink/ButtonLink';
import { useHero } from '../../hooks/useHero'; // Importação do hook useHero
import {
	BorderRadiuses,
	Colors,
	Shadows,
...
export function HeroCard({ secretIdentity, name, picture, universe, id }) {
	const { getHeroAvaliation } = useHero(); // Chamada do hook para obter a avaliação do herói
	const heroAvaliation = getHeroAvaliation(id); // Armazenamento da avaliação do herói
			...
			<InformationGrid p={Spaces.TWO} mb={Spaces.ONE_HALF}>
				<Box>
					...
					<Description as="div" color={Colors.GRAY_700}>
						...
						{/* Exibe a avaliação do herói ou hífen se não houver */}
					</Description>
				</Box>
				<HeroAvatar src={picture} />
			</InformationGrid>
